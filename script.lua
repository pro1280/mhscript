...
local window_combat = library.window("Combat")
local window_player = library.window("Player")
local silentaura = library.window("杀戮光环设置")
local fling = library.window("修改僵尸设置")
fling.hide()
silentaura.hide()
window_player.toggle("Keep Backpack",false,function(val)
    flags.backpack = val
    if val then
    while(flags.backpack) do
    wait(0.001)
    pcall(function()
    --game:GetService('StarterGui'):SetCoreGuiEnabled('Backpack', true)
    plr.PlayerGui:FindFirstChild("BackpackGui").Enabled = true
    char:FindFirstChild("Grabbed").Value = 0
      end)
    end
    repeat task.wait() until flags.backpack
    end
end)
...
window_combat.toggle("Modify Zombies",false,function(val)
    flags.modzombie = val
    if val then
